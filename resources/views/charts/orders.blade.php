...
@section('content')
    <div class="card card-custom">
        <div class="card-body">
            <form class="form"  action="{{ route('orders-chart') }}" method="get">
            {{ csrf_field() }}
            <!--begin::Search Form-->
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        data.addColumn('number', 'Registered Orders ');

        data.addRows([
                @foreach($orders as $order)
            ["{{ date( 'Y-m-d',strtotime($order->date) ) }}", {{ $order->orders }}],
            @endforeach
        ]);

        // Set chart options
        var options = {'title' : 'Number of Orders Received Per Day.',
            hAxis: {
                title: 'Date'
            },
            vAxis: {
